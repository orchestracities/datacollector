...
public class OIDCUserAuthenticator extends AbstractSSOAuthenticator {
  private static final Logger LOG = LoggerFactory.getLogger(OIDCUserAuthenticator.class);
  ...
  public OIDCUserAuthenticator(SSOService ssoService, @NotNull Configuration conf) {
    super(ssoService);
    this.conf = conf;

    this.oidcTokenURL = conf.get(OIDCService.OIDC_AUTHORIZATION_URL_CONFIG, null);
    this.isDataCollector = true;
    this.doMetaRedirectToSso = conf.get(HTTP_META_REDIRECT_TO_SSO, false);

    if (StringUtils.isNotEmpty(this.oidcTokenURL) && this.oidcTokenURL.endsWith("/")) {
      this.oidcTokenURL = this.oidcTokenURL.substring(0, this.oidcTokenURL.length() - 1);
    }
  }
  ...
  StringBuffer getRequestUrl(HttpServletRequest request, Set<String> queryStringParamsToRemove) {
    ...
    if (this.oidcTokenURL != null && !isDataCollector) {
      requestUrl = new StringBuffer(this.oidcTokenURL);
  ...
  Cookie createAuthCookie(HttpServletRequest httpReq, String authToken, long expiresMillis) {
    ...
    if (isDataCollector) {
      ...
      authCookie.setSecure(oidcTokenURL.startsWith("https"));
    }
