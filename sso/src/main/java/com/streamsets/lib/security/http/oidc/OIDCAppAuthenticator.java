...
public class OIDCAppAuthenticator extends AbstractSSOAuthenticator {
  private static final Logger LOG = LoggerFactory.getLogger(OIDCAppAuthenticator.class);

  public OIDCAppAuthenticator(SSOService ssoService) {
    super(ssoService);
  }
      ...
      if (((HttpServletRequest) request).getHeader(SSOConstants.X_REST_CALL) == null) {
        ...
        if (authToken == null) {
          ...
          try {
            ...
            if (principal != null) {
              ret = new OIDCAuthenticationUser(principal);
            } else {
